    ...
    try {
      if (tipoEvento === "1") {
        ...
        const responseEventos = await api.get(
          `/PresencaEvento/${userData.userId}`
        );
  ...
  async function loadMyComentary(idUsuario, idEvent) {
    try {
      const response = await api.get(
        `/ComentarioEvento/BuscarPorIdUsuario/${idUsuario}/?idUsuario=${idUsuario}&idEvento=${idEvent}`

        //rota funcionando: /ComentarioEvento/BuscarPorIdUsuario/5b8fb16c-f4ea-4809-bc89-1354d70e07f7?idUsuario=81c3e77d-a05d-4872-14e1-08dbf77be94c&idEvento=5b8fb16c-f4ea-4809-bc89-1354d70e07f7  *obs: exibe essa descricao p todos os comentarios de tds os eventos*
      );
      // const response2 = await api.get(
      //   `/ComentarioEvento/BuscarPorIdUsuario/${idUsuario}/${idEvent}`
      // );

      console.log(response.data);
      setComentarioDescricao(response.data.descricao);
    } catch (error) {}
  }
  ...
  async function handleConnect(
    ...
    if (whatTheFunction === "connect") {
      try {
        ...
        setNotifyUser({
          titleNote: "Atenção",
          textNote: `Não foi conectar ao evento..`,
          imgIcon: "danger",
          imgAlt:
            "Imagem de ilustração de sucesso. Moça segurando um balão com símbolo de confirmação ok.",
          showMessage: true,
    ...
    try {
      ...
      //ESSA DESGRAÇA N TÁ NO PADRÃO RESTFUL, POR ISSO A URL TÁ DIFERENTE, POHA!
      const promiseDelete = await api.delete(
        `/PresencaEvento/?id=${idPresencaEvento}`
      );
      ...
      setNotifyUser({
        titleNote: "Atenção",
        textNote: `Não foi possível desconectar do evento!.`,
        imgIcon: "danger",
        imgAlt:
          "Imagem de ilustração de sucesso. Moça segurando um balão com símbolo de confirmação ok.",
        showMessage: true,
