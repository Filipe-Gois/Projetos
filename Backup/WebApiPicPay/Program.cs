...
.AddJwtBearer("JwtBearer", options =>
{
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        IssuerSigningKey = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes("picpay-chave-autenticacao-webapi")),

        //Valida o tempo de expiração do token
        ClockSkew = TimeSpan.FromMinutes(5),

        //De onde está vindo (qual o issure)
        ValidIssuer = "picpay.webApi",

        //Para onde está indo (qual o audiece)
        ValidAudience = "picpay.webApi"
    };
